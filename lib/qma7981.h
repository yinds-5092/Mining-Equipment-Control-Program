...
// 返回: none.
//========================================================================
void qma7981_write_reg(uint8 reg,uint8 data)
{
    hardiic_write_nbyte(QMA7981_I2C_SLAVE_ADDR,reg,&data,1);
}
...
uint8 qma7981_initialize()
{
	...
	{	
		...
		if(data[0] == 0xff)
		{
			qma7981_delay(data[1]);
		}
		else
		{
			if(data[0] == QMA7981_REG_RANGE)
			{
				if(data[1] == QMA7981_RANGE_4G)
					g_QMA7981.lsb_1g = 2048;
				else if(data[1] == QMA7981_RANGE_8G)
					g_QMA7981.lsb_1g = 1024;
				else if(data[1] == QMA7981_RANGE_16G)
					g_QMA7981.lsb_1g = 512;
				else if(data[1] == QMA7981_RANGE_32G)
					g_QMA7981.lsb_1g = 256;
				else
					g_QMA7981.lsb_1g = 4096;
			}
			qma7981_write_reg(data[0],data[1]);
...
// 返回: none.
//========================================================================
void qma7981_read_raw_xyz(int32 *data)
{
	uint8 databuf[6] = {0}; 	
...
uint32 qma7981_read_stepcounter()
{
    ...
    qma7981_read_reg(QMA7981_STEP_CNT_L, data, 2);
	qma7981_read_reg(QMA7981_STEP_CNT_M, &data[2], 1);
