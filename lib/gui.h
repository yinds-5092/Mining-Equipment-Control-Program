...
void calendar_circle_clock_drawpanel(uint16 x,uint16 y,uint16 size,uint16 d)
{
	...
	tft_gui_circle(x, y, r, TFT_LCD_WHITE, 1);
	tft_gui_circle(x, y, r-4, TFT_LCD_BLACK, 1);

	for(i=0;i<60;i++)   //画秒钟格
	{ 
		px0=sx+r+(r-4)*sinf((app_pi/30)*i); 
		py0=sy+r-(r-4)*cosf((app_pi/30)*i); 
		px1=sx+r+(r-d)*sinf((app_pi/30)*i); 
		py1=sy+r-(r-d)*cosf((app_pi/30)*i);  
		tft_lcd_draw_line1(px0,py0,px1,py1,0, TFT_LCD_WHITE);
	}
	for(i=0;i<12;i++)   //画小时格
	{ 
		px0=sx+r+(r-5)*sinf((app_pi/6)*i); 
		py0=sy+r-(r-5)*cosf((app_pi/6)*i); 
		px1=sx+r+(r-d)*sinf((app_pi/6)*i); 
		py1=sy+r-(r-d)*cosf((app_pi/6)*i);  
		tft_lcd_draw_line1(px0,py0,px1,py1,2, TFT_LCD_YELLOW);
	}
	for(i=0;i<4;i++)    //画3小时格
	{ 
		px0=sx+r+(r-5)*sinf((app_pi/2)*i); 
		py0=sy+r-(r-5)*cosf((app_pi/2)*i); 
		px1=sx+r+(r-d-3)*sinf((app_pi/2)*i); 
		py1=sy+r-(r-d-3)*cosf((app_pi/2)*i);  
		tft_lcd_draw_line1(px0,py0,px1,py1,2, TFT_LCD_YELLOW);
	}
	//gui_circle(x,y,d/2,WHITE);		//画中心圆
	tft_gui_circle(x, y, d/2, TFT_LCD_WHITE, 1);
}
...
///////////////////////////////////////////////
	...
	temp+=oldhour;
	px0=sx+r+(r-3*d-7)*sinf((app_pi/6)*temp); 
	py0=sy+r-(r-3*d-7)*cosf((app_pi/6)*temp); 
	px1=sx+r+r1*sinf((app_pi/6)*temp); 
	py1=sy+r-r1*cosf((app_pi/6)*temp); 
	...
	temp+=oldmin;
	//清除上一次数据
	px0=sx+r+(r-2*d-7)*sinf((app_pi/30)*temp); 
	py0=sy+r-(r-2*d-7)*cosf((app_pi/30)*temp); 
	px1=sx+r+r1*sinf((app_pi/30)*temp); 
	py1=sy+r-r1*cosf((app_pi/30)*temp); 
	tft_lcd_draw_line1(px0,py0,px1,py1,1, TFT_LCD_BLACK);
	//清除秒钟
	r1=d/2+3;
	//清除上一次数据
	px0=sx+r+(r-d-7)*sinf((app_pi/30)*oldsec); 
	py0=sy+r-(r-d-7)*cosf((app_pi/30)*oldsec); 
	px1=sx+r+r1*sinf((app_pi/30)*oldsec); 
	py1=sy+r-r1*cosf((app_pi/30)*oldsec); 
...
///////////////////////////////////////////////
	...
	r1=d/2+3;
	//显示新的秒钟
	px0=sx+r+(r-d-7)*sinf((app_pi/30)*sec); 
	py0=sy+r-(r-d-7)*cosf((app_pi/30)*sec); 
	px1=sx+r+r1*sinf((app_pi/30)*sec); 
	py1=sy+r-r1*cosf((app_pi/30)*sec); 
...
void oscilloscope_axis(uint16 x,uint16 y,uint16 size,uint16 d)
{
	...
	while(px0 <= (lcddev.width-size))
	{
		...
		{
			c1 = c1 + size * 5;
			tft_gui_circle(px0, c1, 1, TFT_LCD_WHITE, 1);
		}
		while(c2 >= size*5)
		{
			c2 = c2 - size * 5;
			tft_gui_circle(px0, c2,1, TFT_LCD_WHITE,  1);
	...
	while(py0 <= lcddev.height-size)
	{
		...
		{
			c1 = c1 + size * 5;
			tft_gui_circle(c1, py0, 1, TFT_LCD_WHITE, 1);
			
		}
		while(c2 >= size*5)
		{	
			c2 = c2 - size * 5;
			tft_gui_circle(c2, py0, 1, TFT_LCD_WHITE, 1);
		}
		c1 = x;
		c2 = x;
	}
