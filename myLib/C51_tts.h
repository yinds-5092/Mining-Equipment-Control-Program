...
static uint8  TTS_QUERY_CMD[] = {0xFD, 0x00, 0x02, 0x21, 0x00};
...
{
    uart_init(uart_n, rx_pin, tx_pin, baud, tim_n);
}
...
void TTS_stop(UART_Name uart_n)
{
    ...
    for(i = 0; i < 4; i++)
    {
        asr_tts_buf[i] = TTS_STOP_CMD[i];
    }
    
    tts_writeData(uart_n, asr_tts_buf, 4);
}
...
void TTS_resume(UART_Name uart_n)
{
    ...
    for(i = 0; i < 4; i++)
    {
        asr_tts_buf[i] = TTS_RESUME_CMD[i];
    }
...
uint8 TTS_queryState(UART_Name uart_n)
{
    ...
    // 发送查询命令
    tts_writeData(uart_n, query_buf, 5);
